...
public class Topo {
    public static double Azimut2(double xe,double ye,double xv,double yv){
        double xx,yy;
        double az = 0.0;
        xx = xv-xe;
        yy = yv-ye;
        if (xx == 0.0 && yy == 0.0) return 0.0;
        //if (yy == 0.0 && xx > 0.0)  return 100.0;
        //if (yy == 0.0 && xx < 0.0)  return 300.0;
        //az = Math.atan(xx/yy) * 200 / Math.PI;
        az = Math.atan2(xx,yy) * 200 / Math.PI;
        if (az < 0.0) az += 400;
        return az;
    }
